...
export const repeatDemo = () => {
    repeat(function() {console.log("Hello, world!")}, 10, [])
};
...
export const filter = (arr, test) => {
    let fail_arr = new Array()
    let pass_arr = new Array()
    ...
    let ret_obj = {fail: fail_arr, pass: pass_arr}
    return ret_obj
};
...
export const hasExactly = (arr, test, n) => {
    let test_obj = filter(arr, test)
    if(test_obj.pass.length === n) {
        return true
    } else {
        return false
    }
